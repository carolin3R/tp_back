...
class QuestionIndexViewTests(TestCase):
    ...
    def test_past_question_without_choice(self):
        ...
        index page.
        """
        question = create_question_without_choice(question_text="Past question without choice.", days=-30)
        response = self.client.get(reverse("polls:index"))
        self.assertQuerySetEqual(response.context["latest_question_list"], [])
    ...
    def test_future_question_without_choice(self):
        ...
        the index page.
        """
        create_question_without_choice(question_text="Future question.", days=30)
        response = self.client.get(reverse("polls:index"))
        self.assertQuerySetEqual(response.context["latest_question_list"], [])
    ...
    def test_future_question_and_past_question_without_choice(self):
        """
        Even if both past and future questions exist, only past questions
        are displayed.
        """
        question = create_question_without_choice(question_text="Past question.", days=-30)
        create_question_without_choice(question_text="Future question.", days=30)
        response = self.client.get(reverse("polls:index"))
        self.assertQuerySetEqual(response.context["latest_question_list"], [])
...
class QuestionDetailViewTests(TestCase):
    ...
    def test_future_question_without_choice(self):
        ...
        returns a 404 not found.
        """
        future_question = create_question_without_choice(question_text="Future question.", days=5)
        url = reverse("polls:detail", args=(future_question.id,))
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)
    ...
    def test_past_question_without_choice(self):
        past_question = create_question_without_choice(question_text="Past Question without choice.", days=-5)
        url = reverse("polls:detail", args=(past_question.id,))
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)
